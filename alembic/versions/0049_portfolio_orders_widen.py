...
def upgrade() -> None:
    ...
    op.execute(
        sa.text(
            f"""
            ALTER TABLE {SCHEMA}.portfolio_operations
    ...
    op.execute(
        sa.text(
            f"DROP INDEX IF EXISTS {SCHEMA}.ix_portfolio_orders_order_id"
        )
    )
    op.execute(
        sa.text(
            ...
            ON {SCHEMA}.portfolio_orders (account_id, order_id)
            """
        )
    )
    op.execute(
        sa.text(
            ...
            ON {SCHEMA}.portfolio_orders (account_id, status)
            """
        )
    )
...
def downgrade() -> None:
    op.execute(sa.text(f"DROP INDEX IF EXISTS {SCHEMA}.ix_portfolio_orders_account_status"))
    op.execute(sa.text(f"DROP INDEX IF EXISTS {SCHEMA}.uq_portfolio_orders_account_order_id"))
    op.execute(
        sa.text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS ix_portfolio_orders_order_id
            ON {SCHEMA}.portfolio_orders (order_id)
            """
        )
    )
    op.execute(
