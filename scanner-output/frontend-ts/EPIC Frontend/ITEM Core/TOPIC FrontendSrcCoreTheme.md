...
- Вверх по дереву:
  - ITEM: [[Graph/MOC ITEM — frontend-ts — EPIC Frontend — ITEM Core]]
  - EPIC: [[Graph/MOC EPIC — frontend-ts — EPIC Frontend]]
  - Корень: [[Graph/MOC Root — frontend-ts]]
  - Индекс: [[Graph/Index|Graph / Index]]

Остальные заметки этого ITEM перечислены в [[Graph/MOC ITEM — frontend-ts — EPIC Frontend — ITEM Core]] (избегаем сотен дублей ссылок в каждом файле).
