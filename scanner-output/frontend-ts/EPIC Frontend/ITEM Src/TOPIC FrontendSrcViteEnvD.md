---
gin_graph:
  version: 1
  root: "frontend-ts"
  epic: "EPIC Frontend"
  item: "ITEM Src"
tags:
  - gin-graph
  - gin-scanner
  - gin-root/frontend-ts
  - gin-epic/epic-frontend
  - gin-item/item-src
---

## Связи графа

> Авто: `scripts/obsidian_graph_refresh.py` · обновление: [[Graph/Index|полный индекс MOC]]

- Вверх по дереву:
  - ITEM: [[Graph/MOC ITEM — frontend-ts — EPIC Frontend — ITEM Src]]
  - EPIC: [[Graph/MOC EPIC — frontend-ts — EPIC Frontend]]
  - Корень: [[Graph/MOC Root — frontend-ts]]
  - Индекс: [[Graph/Index|Graph / Index]]

Остальные заметки этого ITEM перечислены в [[Graph/MOC ITEM — frontend-ts — EPIC Frontend — ITEM Src]] (избегаем сотен дублей ссылок в каждом файле).

<!-- gin_graph:end -->
 Исходный модуль `frontend/src/vite-env.d.ts` — автоматическая разметка для Obsidian Source Scanner.

