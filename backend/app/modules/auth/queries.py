...
def build_get_user_by_id_query(include_email: bool = True, include_phone: bool = True) -> str:
    ...
    if include_email:
        base_query += """,
            (SELECT email FROM ganaly.user_email 
    ...
    if include_phone:
        base_query += """,
            (SELECT phone FROM ganaly.user_phone 
             WHERE user_id = u.id AND (valid_to IS NULL OR valid_to > CURRENT_TIMESTAMP)
...
def build_get_user_by_id_simple_query() -> str:
    ...
    return """
           SELECT id, login FROM ganaly.user WHERE id = :user_id \
           """
...
def build_get_user_with_token_query() -> str:
    ...
    return """
           SELECT
               ...
               t.expires_at
           FROM ganaly.user u
                    JOIN ganaly.user_token t ON u.id = t.user_id
