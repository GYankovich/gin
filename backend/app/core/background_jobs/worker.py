...
async def _heartbeat_live_session(job_id: UUID) -> None:
    interval = float(settings.LIVE_SESSION_HEARTBEAT_SECONDS)
    while True:
        await asyncio.sleep(interval)
        db = SessionLocal()
        try:
            touch_background_job(db, job_id)
            db.commit()
        except Exception as exc:
            db.rollback()
            logger.warning("live session heartbeat failed job_id=%s: %s", job_id, exc)
        finally:
            db.close()


async def _execute_job(job_id: UUID, job_type: str, payload: dict) -> None:
    if job_type != "live_trading_session":
        await execute_job_handler(job_type, payload)
        return

    hb_task = asyncio.create_task(
        _heartbeat_live_session(job_id),
        name=f"live-session-hb-{job_id}",
    )
    try:
        await execute_job_handler(job_type, payload)
    finally:
...
class LaneWorkerPool:
    def __init__(self, lane: str, concurrency: int, *, force_lease: bool = False):
        self.lane = lane
    ...
    async def _lease_heartbeat_loop(self) -> None:
        ...
        while self._running and self._lease_worker_id is not None:
            await asyncio.sleep(interval)
            db = SessionLocal()
            try:
                ok = touch_worker_lease(db, lane=self.lane, worker_id=self._lease_worker_id)
                db.commit()
                if not ok:
                    logger.error(
                        "lane=%s lost worker lease worker_id=%s — stopping pool",
                        self.lane,
                ...
                db.rollback()
                logger.warning("lane=%s worker lease heartbeat failed: %s", self.lane, exc)
            finally:
                db.close()

    def _acquire_lease_or_raise(self) -> None:
        db = SessionLocal()
        try:
            lease = try_acquire_worker_lease(db, lane=self.lane, force=self.force_lease)
            db.commit()
            self._lease_worker_id = lease["worker_id"]
            logger.info(
                "lane=%s acquired worker lease worker_id=%s host=%s pid=%s force=%s",
                self.lane,
                lease["worker_id"],
                lease.get("hostname"),
                lease.get("pid"),
    ...
    def _release_lease(self) -> None:
        if self._lease_worker_id is None:
            return
        db = SessionLocal()
        try:
            release_worker_lease(db, lane=self.lane, worker_id=self._lease_worker_id)
            db.commit()
            logger.info("lane=%s released worker lease worker_id=%s", self.lane, self._lease_worker_id)
        except Exception as exc:
            db.rollback()
            logger.warning("lane=%s failed to release worker lease: %s", self.lane, exc)
        finally:
            db.close()
            self._lease_worker_id = None
    ...
    async def start(self) -> None:
        ...
        self._lease_hb_task = asyncio.create_task(
            ...
            name=f"worker-lease-hb-{self.lane}",
        )
    ...
    async def stop(self) -> None:
        if not self._running and self._lease_worker_id is None:
            return
        self._running = False
        if self._lease_hb_task is not None:
            self._lease_hb_task.cancel()
            try:
                await self._lease_hb_task
            except asyncio.CancelledError:
                pass
            self._lease_hb_task = None
        for task in self._tasks:
            task.cancel()
        if self._tasks:
    ...
    def _sweep_stale_jobs(self) -> None:
        ...
        try:
            ...
            db_sweep.rollback()
            logger.warning("lane=%s stale sweep failed: %s", self.lane, exc)
        finally:
            db_sweep.close()
        ...
        while self._running:
            try:
                ...
                try:
                    ...
                    await _execute_job(job_id, job_type, dict(job.get("payload") or {}))
                    ...
                    try:
                        fail_background_job(
                            db_cancel,
                            job_id,
                            "cancelled (worker stop)",
                            message="failed (worker cancelled)",
                        )
                        db_cancel.commit()
                    except Exception as exc:
                        db_cancel.rollback()
                        logger.warning(
                            "failed to mark cancelled job lane=%s id=%s: %s",
                            self.lane,
                            job_id,
                            exc,
                        )
...
async def run_standalone_lane_worker(lane: str, *, force_lease: bool = False) -> None:
